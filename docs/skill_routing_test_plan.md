...
依次运行 CLI，并在每次请求后查看 Event 日志中的 `routing.resolved`：
...
- LLM I/O 的 `llm.request.instructions` 不包含其他 Skill 的 `Loaded skill:` 标记。
- 工具调用和最终答案符合原有业务行为。
...
- 路由选择 Finance。
- Event 的 `tool.completed` 保存 Action 与压缩信息。
- `context_compaction.strategy` 为 `reference`，包含非空 `ref_id`。
- 模型收到摘要而不是全部记录。
...
- 完整结果不被再次压缩。
- 最终回答中的日期、金额和描述与 Context Ref 完整结果一致。
...
对同一版本分别测试普通对话、单领域和四领域输入，记录 Event 中的
`prompt_chars`，并查看 LLM I/O instructions。
...
- Event、LLM I/O 和 Application 日志仍能配对解释 Agent 行为。
...
1. `routing.resolved` 是否选中正确 Skill。
2. LLM I/O instructions 是否真正加载了对应正文。
3. 模型是否选择了正确 Tool 和参数。
4. Event Action result 和 Context Ref 完整结果是否正确。
...
- 每次失败都能通过 Event 与 Application 日志定位到具体层级。
