...
from app.observability.telemetry import EndSpanInput, RequestTelemetry, StartSpanInput
...
class RequestLogIdentity(Protocol):
    run_id: str
    session_id: str
    turn_id: str
...
class FileTraceExporter:
    """Append canonical Trace Contract records to one session JSONL file."""
    ...
    def export(self, records: tuple[TraceRecordItem, ...]) -> None:
        for record in records:
            entry = asdict(record)
            entry["record_type"] = _trace_record_type(record)
...
class FileAnnotationSink:
    """Append shared diagnostic/evaluation annotations to session JSONL."""
    ...
    def record(self, annotation: AnnotationRecord) -> None:
        entry = asdict(annotation)
        entry["record_type"] = "annotation"
        self._writer.append(entry)
...
class RequestLlmLog:
    ...
    def __init__(
        self,
        writer: LlmLogWriter | None,
        request: RequestLogIdentity,
    ...
    def record(
        ...
        *,
        ...
        interaction: LogLlmInteraction | None = None
        interaction_written = False
        if self._writer is not None:
            try:
                interaction = LogLlmInteraction(
                ...
                self._writer.append(interaction)
                interaction_written = True
            except Exception:
                logging.getLogger("lifeops.llm").error("LLM interaction export failed")
        if self._telemetry is not None:
            try:
                handle = self._telemetry.start_span(
                    StartSpanInput(
                        ...
                        attributes={
                            ...
                            "llm.model": model,
                            "llm.interaction.sequence": self._seq,
                            **{
                                f"llm.usage.{name}": value
                                for name, value in usage.items()
                                if name
                                in {"input_tokens", "output_tokens", "total_tokens"}
                            },
                        },
                    )
                )
                if interaction_written and interaction is not None:
                    self._telemetry.add_artifact_reference(
                        ArtifactReference(
                            ...
                            storage_kind="jsonl",
                            safe_reference=f"llm.jsonl#id={interaction.id}",
                ...
                self._telemetry.end_span(
                    EndSpanInput(
                        handle,
                        TraceStatus.OK if status == "ok" else TraceStatus.ERROR,
                        error_code,
                    )
                )
            except Exception:
                logging.getLogger("lifeops.llm").error("LLM trace recording failed")
...
class SessionLogWriter:
    ...
    def __init__(
        ...
        *,
        ...
        self.trace_exporter = FileTraceExporter(self.session_dir / "traces.jsonl")
        self.annotation_sink = FileAnnotationSink(
            self.session_dir / "annotations.jsonl"
        )
...
def _trace_record_type(record: TraceRecordItem) -> str:
    name = type(record).__name__
    words: list[str] = []
    start = 0
    for index, character in enumerate(name):
        if index and character.isupper():
            words.append(name[start:index].lower())
            start = index
    words.append(name[start:].lower())
    return "_".join(words).removesuffix("_record")
