...
class ReactExecutor:
    ...
    def execute(
        ...
        try:
            context_contributions = self._context_provider.load(request)
            memory_contributions = self._memory_provider.load(request)
        ...
        return self._complete(result, trace)
    ...
    def _complete(
        ...
        try:
            self._feedback_sink.record(result)
        except Exception:
            _trace_hook_failure(trace, "feedback")
        try:
            self._recovery_hook.on_stop(result)
        except Exception:
