...
| `app/runtime/context_engine.py` | LLM 输入组装入口；当前为 pass-through，并输出 Context Unit 与预算报告 |
...
当前已有 pass-through ContextEngine 和 Tool Observation 压缩，还没有“长期对话窗口压缩”。二者不要混淆。

### Prompt与历史消息

- `instructions` 每轮重新生成，通过独立参数发送。
- `Agent.messages` 保存跨Chat历史，包括Function Call和Tool Observation。
- 发送给模型的 `input` 已由 `ContextEngine.assemble()` 生成；当前阶段仍原样透传完整 `Agent.messages`。
- `ContextEngine` 会把历史切分为 user、assistant、tool 或 protected system_note 单元，并在日志中报告消息数、单元数、近似token数、tool schema体积和 protected unit 数。
